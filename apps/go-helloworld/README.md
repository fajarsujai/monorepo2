...
Option A – via moon command: `moon go-helloworld:install-mockery`
...
```bash
moon run go-helloworld:generate-swagger
```

## Production

### Instrumentation
Tracing is enabled exclusively in the production environment. Set `APP_ENVIRONMENT` to `production` to activate tracing. Alternatively, you may customize the tracing rules in `apps/go-helloworld/config/tracer.go`.

For instructions on customizing span tracing, please refer to the example located at:
- `apps/go-helloworld/internal/rest/user.go`
